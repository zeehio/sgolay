...
benchmark_helper <- function(signal_length, num_signals, filter_length, rowwise) {
  ...
  bm <- bench::mark(
    signal = {sgolayfilt(x, filt, engine = "filter", rowwise = rowwise)},
...
ggplot(dplyr::filter(out, rowwise == FALSE)) + 
  geom_col(
    aes(
      x = sprintf("%d", as.numeric(signal_length)),
      y = 1 + as.numeric(cpu_min)*1000,
      fill = engine
    ),
...
```{r}
ggplot(dplyr::filter(out, engine == "signal")) + 
  geom_col(
    aes(
      x = sprintf("%d", as.numeric(signal_length)),
      y = 1 + as.numeric(cpu_min)*1000,
